...
-- Resurrect: збереження та відновлення сесії
...
-- Автовідновлення при запуску в поточне вікно
wezterm.on('gui-startup', function(cmd)
	local tab, pane, window = wezterm.mux.spawn_window(cmd or {})
	-- Завантажуємо останній збережений workspace (назва 'default' або змінити на свою)
...
config.keys = {
	...
	},
	{
		...
		action = wezterm.action_callback(function(win, pane)
			...
			win:toast_notification('WezTerm', 'Сесію збережено', nil, 2000)
		end),
	},
	-- Resurrect: відновити сесію через fuzzy finder
	{
		key = 'r',
		mods = 'CTRL|ALT',
		action = wezterm.action_callback(function(win, pane)
